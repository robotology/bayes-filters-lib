\label{upf_md__home_xenvre_robot_code_bfl_robotology_doc_ex_upf}%
\Hypertarget{upf_md__home_xenvre_robot_code_bfl_robotology_doc_ex_upf}%
...
\DoxyCodeLine{162     std::unique\_ptr<GaussianPrediction> upf\_prediction = utils::make\_unique<UKFPrediction>(std::move(wna), state\_size, alpha, beta, kappa);}
...
\DoxyCodeLine{203     std::unique\_ptr<GaussianCorrection> upf\_correction = utils::make\_unique<UKFCorrection>(std::move(simulated\_linear\_sensor), state\_size, alpha, beta, kappa);}
