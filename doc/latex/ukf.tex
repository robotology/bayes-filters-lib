\label{ukf_md__home_xenvre_robot_code_bfl_robotology_doc_ex_ukf}%
\Hypertarget{ukf_md__home_xenvre_robot_code_bfl_robotology_doc_ex_ukf}%
...
\DoxyCodeLine{134     std::unique\_ptr<UKFPrediction> ukf\_prediction = utils::make\_unique<UKFPrediction>(std::move(wna), state\_size, alpha, beta, kappa);}
\DoxyCodeLine{135 }
\DoxyCodeLine{136 }
...
\DoxyCodeLine{161     std::unique\_ptr<UKFCorrection> ukf\_correction = utils::make\_unique<UKFCorrection>(std::move(simulated\_linear\_sensor), state\_size, alpha, beta, kappa);}
\DoxyCodeLine{162 }
\DoxyCodeLine{163 }
